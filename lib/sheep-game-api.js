...
const {sleep, getUnitSize, unitToEmoji, createLobbyUrl, sumUnitsPrice} = require("./utils");

const CHECK_TIMER_DELAY = 333;
...
class SheepGameApi {
    ...
    addListeners(url, lobby) {
        ...
        this.connection.on("*", async (event, data) => {
            ...
            switch (event) {
                case "extraSync":
                case "setGame":
                    data.users ? this.state.users = data.users : null;
                    data.units ? this.state.units = data.units : null;
    ...
    getPhaseByLinePosition() {
        let {team, line, units, users} = this.state;
        let next = this.getNextStructure();
        let [unit, pos = {y: 0}] = next || [];
        ...
        let balance = (users.find(({id}) => id == this.id) || {}).balance
    ...
    _renderUnits() {
        if (typeof this.state.units === "undefined" || this.state.units.length === 0) return;
        let board = new Array(11).fill(new Array(7).fill("  "));
                ...
                for (let x = 0; x < unitSize[0]; x++) {
                    for (let y = 0; y < unitSize[1]; y++) {
                        ...
                        if (typeof board[i] != "undefined" && typeof board[i][j] != "undefined") {
                            ...
                            if (unit.y % 50 === 0 && typeof board[i + 1] != "undefined") {
                                board[i + 1][j] = emoji;
                            }
                        }
                    }
        ...
        console.log(((this.state || {}).users || []).map(u => (`#${u.team} @id${u.id} [${u.deck.map(d =>
            unitToEmoji(d)).join(",")}]: ${u.balance.toFixed(0)} nw: ${sumUnitsPrice(((this.state || {}).units.filter(({team}) => team === u.team)))}`)).join("\n"));
    }
}
